...
import drawing_images_programme
import pandas as pd
import numpy as np
import matplotlib
import threading
...
with open("Configuration files/Configurate default read in variables.yaml", "r") as defaults:
    default_config_variables = yaml.load(defaults, Loader=yaml.FullLoader)

with open("Configuration files/Configurate variables.yaml", "r") as variables:
...
def get_list_of_csv_files(path_input):
    ...
    # check if the input is a folder and put the names of the csv files within this folder into a list
        ...
        print("The location you gave isn't a csv file or a folder of csv files, please run this programme again with a valid file name.")
        exit(0)
...
def get_variables_from_command_line():
    ...
    # csv path
    parser.add_argument(dest="path", nargs="?", type=str, help="The path to the csv files or the folder of csv files needed in this programme.")
    ...
    if args.background_image_name:
        ...
        try:
            ...
            print("\nThis background image does not exist, please run this programme again with a valid path to an image.")
            exit(0)
    ...
    if args.shapes_filename:
        ...
        if os.path.isfile(args.shapes_filename):
            try:
                ...
                print("Please run this programme again ensuring the format of the file is standard json format.")
                exit(0)

        else:
            print("\nThe file containing the list of the shapes and coordinates to use for the areas drawn on the image given does not exist.")
            print("Please run this programme again with a valid filename.")
            exit(0)
    ...
    if args.events_file:
        try:
            ...
            print(" Please run the programme again with a valid file name")
            exit(0)
        ...
        try:
            ...
            print("Please run the programme again with a valid text file containing 2 tokens per line, a second and then text for an event seperated by a space.")
            exit(0)
    ...
    if args.folder_of_videos:
        ...
        try:
            ...
            print("\nThe folder_of_videos that you entered is not a valid folder name. Please run the programme again with a valid folder name")
            exit(0)
...
def obtain_variables_from_user():
    ...
    if background_image_name:
        ...
        try:
            ...
            print("\nThe path you supplied is invalid. Please run this programme again and enter a valid path to an image.")
            exit(0)
    ...
    if read_in:
        ...
        try:
            ...
            print("\nThis value must be an integer, please run this programme again with a valid video length.")
            exit(0)
    ...
    if read_in:
        ...
        if os.path.isfile(read_in):
            ...
            try:
                ...
                print("\nCould not load in the contents of the file containing the list of the shapes and coordinates to use for the areas drawn on the image.\nPlease run this programme again ensuring the format of the file is standard json format.")
                exit(0)

        else:
            print("\nThe file containing the a list of the shapes and coordinates to use for the areas drawn on the image given does not exist. Please run this programme again with a valid filename.")
            exit(0)
    ...
    if read_in:
        ...
        try:
            ...
            print("\nThis value must be a valid name of a colour map, please run this programme again fixing this error.")
            exit(0)
    ...
    if folder_of_videos:
        try:
            ...
            print("\nThe folder containing the videos that you entered is not a valid folder name. Please run the programme again with a valid folder name")
            exit(0)
    ...
    for sec in seconds:
        if sec in dictionary_of_events:
            ...
            if text_width > x_width:
                print("The event name '" + text + "'' is too long for the text bar at the top, please input a shorter description of the event and re-run the programme.")
                # exit(0)
...
def main():
    ...
    if shapes_filename:
        ...
        list_of_shapes_details = drawing_images_programme.main(len(csv_files), background_image_name, basewidth)
    ...
    if len(csv_files) != len(list_of_shape_arrays):
        print("\nThe number of areas you drew and the number of csvs you supplied do not match. Please re-run the programme drawing the same amount of areas on the image as csvs you supplied.")
        exit(0)

    if len(csv_files) < len(list_of_videos):
        print("\nThe number of videos in the folder supplied is greater than the number of csvs you supplied. Please re-run the programme inputting the same amount or less videos than csvs you supply.")
        exit(0)
